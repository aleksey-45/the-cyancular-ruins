...
class FlyBird:
	const wake_radius: float = 1200.0     # 视野半径,屏幕外可见
	const max_chase_distance: float = 4000.0  # 距玩家超此值不启动任何寻路(直接返程)
	...
	const shoot_range: float = 700.0      # 进入射击距离
	...
	const charge_speed: float = 900.0     # 冲撞速度
	const charge_timeout: float = 2.5     # 冲撞超时 → 自毁
	const charge_damage: int = 5          # 冲撞撞玩家伤害
	const charge_impact: float = 1000.0    # 冲撞冲击力(撞飞玩家水平速度)
