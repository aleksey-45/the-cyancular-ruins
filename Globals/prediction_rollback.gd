...
# C2 rollback 控制器(与引擎解耦,纯逻辑)。移植自原作者 origin/main `core/prediction_rollback.gd`
# (提交 70e4c75「C2 客户端预测 rollback 落地并启用」),接口按本仓库 player/输入源适配。
#
# 被预测 Player 的驱动方式由接入方决定,两种皆可:
#   A) 手动步进(探针/无引擎环境):每帧调 advance(record) —— 内部把玩家 input_source 换成
