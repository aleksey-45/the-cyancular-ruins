...
开发专用 GUI 工具:「填卡 → 生成施工提示词 → 自动调 Claude Code 施工」。**只在 KH-char-weap 分支存在**;`export_presets.cfg` 两处 `exclude_filter` 已含 `DevTools/*`,玩家包不可见。跑法:**双击项目根目录 `启动卡编辑器.bat`**(内部即 `"C:/Godot/Godot_v4.7.1-stable_win64.exe" --path . res://DevTools/card_editor.tscn`,带分支守卫:场景缺失时提示切分支,不会误启游戏)。
