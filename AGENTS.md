...
- **本地玩家 = C2 客户端预测 + 回滚重放(已开启;1v1 + 大乱斗)**:`player.gd` 的 `server_rendered` 模式下跳过全部移动物理,位置/姿态/朝向由快照插值(`apply_server_snapshot` + `_update_server_rendered`),血量/防水/倒地直接采纳;只保留鼠标瞄准/开火/受击反馈等本地视觉。历史根因:C2 对梯子等"边沿+位置敏感"机制与服务器权威打架 → 曾大量回拉,故此前默认关闭;2026-09-11 按用户要求开启。实现:`Globals/prediction_rollback.gd`(`class_name PredictionRollback`,移植自原作者 main `70e4c75`)。开关 = `Settings.pvp_c2_prediction`(默认开,设置页可关;`pvp_client.gd` / `royale_game.gd` 各自在进场景时读进 `_predict`),关掉即回到纯服务器渲染(server_rendered 分支完整保留)。探针 `Tests/c2_reconcile_probe.tscn` / `c2_twin_probe.tscn` 实测 OK。**1v1 与大乱斗均已实装**(大乱斗侧 2026-09-11,带宽 O(N):只给每人补发自己那份整态)。远端对手 = `PlayerReplica` 纯视觉副本(显示对手当前武器并按快照 `aim` 摆枪;**对手重武器预瞄中 → 副本也画出预瞄线**——M82A1 直线激光 / 榴弹抛物线弧,经 `weapon_base.drive_remote_visual` 驱动,不开火不读鼠标,`previewing` 为 false/倒地时收起)。**副本位置插值(重要)**:`player_replica` 位置走**双快照 tick 域 alpha 插值**——缓冲最近若干 canonical,渲染时钟落后最新 1 tick、按真实时间在相邻两快照间线性插值(`apply_snapshot` 带 tick 入缓冲;姿态/朝向/aim/倒地/武器仍按最新快照即时,只有位置平滑落后);时钟只在 tick 域走、不依赖两端时钟同步,丢包/卡顿冻结在最新已收位置,快照续上把时钟重置到最新窗口继续,不回退。插值结果跨接缝取 `toroidal_delta_px` 最短向量后取模回 canonical,再 `anchor_to_nearest` 锚到本地玩家最近副本渲染——每帧直接归位到可见副本,无旧指数/差分追赶。
...
- PvP 固定地图 `map/factory1v1.cyrm`(150×100,`# player 17 65` + `# player2 133 64`;两出生点相距约 2200px 超视野,走近才互见)。由 `server/room_manager.gd` 的 `PVP_MAP` 常量指定;**多人建房页可选 `res://map/` 下的其它图**(房主选择随 `player_options.map` 上报,`resolve_map()` 找不到时回退此图)。地图格式与导入流程见 `docs/ARCHITECTURE.md` §6。
...
1) 启动器会话(最全):双击 start_game_logged.bat / start_server_logged.bat(或拖 exe 上去),
   capture_session.ps1 重定向 stdout/stderr,生成中文 report.txt(结论三态 OK/!/X+问题清单);
2) 后台看守会话:crashlog_capture.py(watch/start/stop/report/selftest,支持 install-task 登录自启)
   按进程前缀看守——**直接双击 exe 启动的运行**也归档到 gamelogs/archive/<stamp>_<exe>/,
   产出 run.json + 同格式中文 report.txt(崩溃时从 Windows 事件日志取异常代码/故障模块/偏移);
   崩溃轮永不清理(非崩溃轮保留最近 100)。监看器对看守会话从 run.json 现场合成中文报告。
...
### 已知文档漂移
**结构性漂移(武器数量/C2 状态/DevTools 分支归属/小地图等件的归属)已校准并汇总在 `docs/ARCHITECTURE.md` §7,新增文件请同步登记到该文档。** 下列为**代码注释级**遗留:
