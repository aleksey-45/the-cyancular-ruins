...
# 主角水中物理:默认沉底,按上上浮到水面(悬停);水平游泳。根每帧显式调用 update(),
...
func update(parent: CharacterBody2D, delta: float, move_mult: Vector2 = Vector2.ONE) -> bool:
	...
	if Input.is_action_pressed("up"):
		var feet_y := parent.global_position.y + Water.feet_offset(parent)
		parent.velocity.y = 0.0 if feet_y <= surface_y + 4.0 else PlayerParams.player_swim_up
	else:
