extends Node
# C2 孪生探针(scene 模式 headless;移植自原作者 tests/pvp_twin_smoke.gd):
# B 每 K tick 被强行搞乱后再 restore_state(A 快照)+ 同输入继续,必须与从不被打断的 A 逐 tick 收敛。
# 漏一个 capture_state 字段 → B 重放与 A 发散 → 探针失败(capture/restore 见 player.gd)。
# 跑法: Godot_console --headless --path . res://Tests/c2_twin_probe.tscn
...
func _ready() -> void:
	...
	print("[c2_twin] 世界 %dx%d 格;玩家出生 %s;计划 %d tick(restore every %d)" % [
...
func _fail() -> void:
	print("C2_TWIN FAIL: %s" % _violation)
...
func _finish() -> void:
	if not _violation.is_empty():
		_fail()
		return
	print("C2_TWIN OK: %d ticks, max pos dev %.4f px, 0 字段发散" % [_tick, _max_pos_dev])
	get_tree().quit(0)
