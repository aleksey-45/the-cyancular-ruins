...
# 模拟 主菜单→(单人面板→开始探索 / 多人) 的真实按钮点击流转,验证切换后场景与脚本报错。
# 由 main_menu._ready 在命令行含 --autotest-sp / --autotest-mp 时挂载,平时零开销。

var mode := ""   # "sp" / "mp"(由 main_menu 经 cmdline 参数注入)
...
func _run() -> void:
	...
	if mode == "level":
		# 直接切 Level0(绕过菜单流转),隔离"菜单演示残留"变量
	...
	if cur != null and cur.get_node_or_null("WorldViewport") != null:
		...
		print("AUTOTEST: WorldViewport size=", wv.size, " render_target_update_mode=", wv.render_target_update_mode)
	...
	if pp != null:
		var rect: Control = pp.get_node_or_null("ShaderRect")
		print("  ShaderRect=", rect, " visible=", rect.visible if rect != null else "-",
				" size=", rect.size if rect != null else Vector2(),
				" mat=", rect.material if rect != null else null)
	# 截图存档(带窗口运行时):root 视口 = 玩家实际看到的画面
	await tree.process_frame
	await tree.process_frame
	var img := tree.root.get_texture().get_image()
	img.save_png("user://autotest_%s.png" % mode)
	print("AUTOTEST[%s]: 截图已存 user://autotest_%s.png" % [mode, mode])
	print("AUTOTEST[%s]: DONE" % mode)
	tree.quit(0)

# 按文字前缀找按钮并触发(浮现动画不阻塞 pressed)
