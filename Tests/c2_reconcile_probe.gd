extends Node
# C2 rollback 控制器进程内探针(scene 模式 headless;移植自原作者 tests/pvp_reconcile_smoke.gd):
#   A = 权威模拟(服务器,1 输入/tick 消费);P = 被预测玩家 + PredictionRollback。
#   ack/整态按人工 D tick 延迟投递到 P;并在 tick E 对 A 注入外部事件(传送=击退/换边等效),
#   断言:常态(无事件)下 P==A 无橡皮筋;事件后 ack 到期 → P 一次性 rollback 重对齐,随后收敛。
# 跑法: Godot_console --headless --path . res://Tests/c2_reconcile_probe.tscn
...
func _ready() -> void:
	...
	# 手动步进:关掉引擎自动 _physics_process,由本探针逐帧驱动
	A.set_physics_process(false)
	P.set_physics_process(false)
	ctrl.bind(P)
	print("[c2_reconcile] D=%d E=%d 计划 %d tick" % [DELAY, EVENT_TICK, TOTAL])
...
func _fail() -> void:
	print("C2_RECONCILE FAIL: %s" % _violation)
	get_tree().quit(1)
...
func _finish() -> void:
	...
	if ctrl.rollback_count() == 0:
		print("C2_RECONCILE FAIL: 外部事件未触发 rollback")
		get_tree().quit(1); return
	if not _post_converged:
		print("C2_RECONCILE FAIL: 事件后未收敛")
		get_tree().quit(1); return
	print("C2_RECONCILE OK: %dtick 内 rollback×%d,maxdev=%.2f px(事件前 %.2f),事件后已收敛" % [
		TOTAL, ctrl.rollback_count(), _max_dev, _max_pre_event_dev])
	get_tree().quit(0)
