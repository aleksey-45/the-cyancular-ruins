...
func _build_weapon(box: VBoxContainer) -> void:
	...
	_row(box, "弹夹容量", _spin(box, "mag_size", 0, 250, 1, "(0=无弹夹)"), "mag_size")
	_row(box, "换弹时间", _spin(box, "reload_time", 0.0, 10.0, 0.1, "s"), "reload_time")
	_row(box, "伤害", _spin(box, "damage", 1, 200, 1))
	_row(box, "击退力度", _spin(box, "impact", 0, 3000, 10))
	_row(box, "分级", _opt(box, "tier", TIER_LABELS))
	_row(box, "全自动", _chk(box, "full_auto", "按住连发"), "full_auto")
	_row(box, "重瞄", _chk(box, "heavy_aim", "按住激光预瞄,松开发射"), "heavy_aim")
	_row(box, "注册槽位", _spin(box, "slot", 0, 9, 1, "(0=纯设计稿,不注册)"))
	box.add_child(DevUIKit.label("弹道(近战类不适用)", 20, Color(0.75, 0.8, 0.85)))
	_row(box, "弹速", _spin(box, "bullet_speed", 100, 4000, 50, "px/s"), "bullet_speed")
	_row(box, "射程", _spin(box, "bullet_range", 50, 3000, 50, "px"), "bullet_range")
	_row(box, "弹丸数", _spin(box, "pellet_count", 1, 24, 1), "pellet_count")
	_row(box, "散布半角", _spin(box, "spread_deg", 0, 45, 0.5, "°"), "spread_deg")
	_row(box, "弹体重力×", _spin(box, "bullet_gravity", 0, 1, 0.05), "bullet_gravity")
	box.add_child(DevUIKit.label("性质参数", 20, Color(0.75, 0.8, 0.85)))
	_row(box, "近战范围", _spin(box, "kp_range", 20, 400, 5, "px"), "kp_range")
	_row(box, "挥砍弧角", _spin(box, "kp_arc", 30, 360, 10, "°"), "kp_arc")
	_row(box, "引信时长", _spin(box, "kp_fuse", 0.1, 5, 0.1, "s"), "kp_fuse")
	_row(box, "爆炸半径", _spin(box, "kp_radius", 50, 900, 10, "px"), "kp_radius")
	_row(box, "爆炸伤害", _spin(box, "kp_edmg", 1, 150, 1), "kp_edmg")
...
func _build_prop(box: VBoxContainer) -> void:
	...
	_row(box, "注册槽位", _spin(box, "slot", 0, 13, 1, "(11=击退 12=吸引 13=烟雾;0=设计稿)"))
	_row(box, "分级", _opt(box, "tier", TIER_LABELS))
	...
	_row(box, "起效延迟", _spin(box, "kp_fuse", 0.0, 5, 0.1, "s(首次碰撞后)"), "kp_fuse")
	_row(box, "作用半径", _spin(box, "kp_radius", 50, 900, 10, "px"), "kp_radius")
	_row(box, "推/吸强度", _spin(box, "kp_force", 0, 9000, 50, "(负=吸引)"), "kp_force")
	_row(box, "烟雾时长", _spin(box, "kp_smoke", 0, 30, 0.5, "s"), "kp_smoke")
	_row(box, "移速惩罚×", _spin(box, "move_penalty", 0.3, 1.0, 0.05))
	_row(box, "跳跃惩罚×", _spin(box, "jump_penalty", 0.3, 1.0, 0.05))
...
func _fill_prop() -> void:
	...
	_f_prop["slot"].value = float(_card.get("slot", 11))
