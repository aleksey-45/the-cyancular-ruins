...
★ **第 3 条与第 1 条有冲突**，见 §3.1 —— spec 里给了一个两全改法，需要你点头。
...
### 3.1 会话令牌（★ 这里与你的裁定有一处偏差，请确认）

**你选的是**「worker 发一次性 session token」；**但同时要求**「回大厅后能回那局」。这两条冲突：**大厅并不知道 worker 发的 token**，所以客户端拿着 token 回大厅时，大厅无法把它对回任何一局。

三个出路：

| | 做法 | 代价 |
|---|---|---|
| **A（推荐）** | **改由大厅生成 token**，随 `go_match` 一起下发；客户端转连 worker 时把它带进 `claim_role(role, name, token)`。worker 存下 `role → token` 供日后核验 | 与你的原话只差"谁生成"；语义不变（仍是一次性、仍只有原客户端拿得到）；大厅天然知道 token，回局查询免费 |
| B | worker 生成，再**主动连回大厅**上报 `role → token`（新增一条 worker→大厅通道） | 多一条常驻连接与一套新协议；且 worker 崩了这条要处理 |
| C | 大厅与 worker **各持一半**（双 token） | 最丑，两处状态要同步失效 |

**spec 余下部分按 A 写。** 若你坚持 worker 生成，请选 B（C 不建议）。

Token 规格：`String`，16 个 hex 字符（`%016x` 于 `randi()` 两次拼接），**每局每 role 一份**，随房间一起生灭。
...
**两种都符合"可被打可被杀"，差别在宽限期的意义**。spec 默认取**照常复活**（更贴合你的原话"随意攻击"），并把宽限期默认值定得短一些（`GRACE_SECONDS = 30`），同时把它做成常量便于调。**若你希望宽限期只惩罚"掉线"而不送分，改成"不复活"即可 —— 这是实施时的一处开关。**
...
2. **1v1 宽限期送分**（§3.8）：默认取"照常复活"时，在线方可在宽限期内刷分赢下整场。
