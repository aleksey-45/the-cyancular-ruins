...
**★ 真机测出的一个崩溃（2026-09-12 晚，用户报告，**尚未定位**）**：大乱斗里 A 把 B 杀了之后
B 的客户端消失、A 随即"胜利"。已确定的结构：A 的胜利是 B 消失的**结果**（`mark_disconnected` →
`online < 2` → `_finish_match()`），所以问题等价于"B 为什么消失"。
**★ 本批探针覆盖不到它**：我的探针里 c1 是**按 K 自杀** —— 自杀在客户端**不经过 `take_hit`**
（没有射手就没有 `hit_event`），客户端只是被 rollback 拉成倒地；而"被枪打死"会走
`_on_hit_event → _local.take_hit(...)`。**这条路径在本批的门里是空的。**

**其他执行期发现**：
- **`tests/royale_soak_probe.gd` 从批次 3 起就解析不过**（引用了批次 3 删掉的 `PvpSession.pending_*`
  与 `clear_pending_payloads()`）——`royale_probe`/`royale_bound_probe` 都改了，漏了它。
  后果：**"机器人追着对手打、直到有人死"这条路径全仓没有自动覆盖**（1v1 三个冒烟都不开火）。
  已于 2026-09-12 修好并跑通（`SOAK: ALL-OK`）；但**同一次跑出的是 `hit_event 0 / kill_event 0`**
