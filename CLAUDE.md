...
- **一局是怎么起来的(大厅 → worker 子进程 → `RoyaleHost`)**:
  ...
  2. 房主发 `royale_start`(或 `royale_start_ai`)→ `_spawn_royale_worker(port, players, ai_roles)` 用 `OS.create_process` 拉起**独立 worker 子进程**,命令行 = `--headless [--path . res://server/server_main.tscn] -- --worker --royale --port P --players N [--ai-roles r,r]`(**每个开关一个独立 argv 项**,`--ai-roles` 的值是逗号串;editor/template_debug 下必须带 `--path`+场景,仅导出 exe 可省。与 `server_main.gd` `_ready` 里的 argv 解析**逐字对应**——`--players` 被 `clampi(..., 2, 8)`,`--ai-roles` 逐 token 取 1..8;两边改一处必须同步改另一处)。进程隔离 = 各局内存隔离,共享全局 `current_grid`/`TileDefs` 不跨局互踩。
  3. worker 的 `--royale` 分支:`_on_role_claimed` 收齐 **`_expected_players - _ai_roles.size()`** 个人类 `claim_role` 即开局(其余角色由 AI 补位);`_process` 另有两条超时梯——**已到 ≥2 人但 20s 仍收不齐** → 按已到人数降级开局(缺席 role 不入局,`RoyaleHost.plan_spawns` 按实际键返回散点以容忍不连续 role);**可用玩家 <2 人**(开局前全掉线)→ **10s 宽限后退出释放端口**(否则 worker 僵死占端口、大厅回收后撞车新对局)。`_on_role_claimed` 另有**报到串线防护**:对局已开始/role 越界/该 role 已被别的 peer 占用 → 直接 `disconnect_peer`(端口复用竞态下迟到的客户端会连到旧 worker,不能让它静默留在局里收快照)。`_begin_match` 是抽出来的函数,1v1 走 `RoomManager.start_match_on`、大乱斗走 **`RoyaleHost.start_on`**。
  4. 端口归还延迟:`ROYALE_PORT_REUSE_DELAY`=**360s**(1v1 的 `WORKER_PORT_REUSE_DELAY`=30s 不够一局最长 5min,沿用会让对局中途端口被发给新 worker → 串线/bind 冲突);`royale_rooms` 已纳入 **`_sweep_stale_rooms`**(大乱斗 `worker_port` 只在开局时分配,唯一归还路径是 `on_peer_left` 的空房分支 → 成员静默挂着不走就永久占端口,500 个端口池会被慢速耗尽);在局中的大乱斗房另加 **`SWEEP_INTERVAL + RoyaleHost.MATCH_TIME`** 宽限(覆盖「阈值 + 整个扫描周期 + 一局」,可证安全;等待中的房不占端口,仍按裸 `MAX_ROOM_AGE` 清)。
