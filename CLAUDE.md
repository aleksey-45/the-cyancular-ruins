...
- 子弹:玩家 `BulletBase`(伤害由 WeaponBase 管);**爆炸弹**(如 `grenade_bullet.tscn`)在 BulletBase 加 @export `explodes/direct_hit_damage/fuse_time/explosion_radius/damage/knockback/visual` —— `explodes=true` 时命中敌人 10 直接伤+立即爆炸、撞墙停驻后 `fuse_time`(0.5s)才爆炸、超射程兜底爆炸。AoE 判定在 `Globals/explosion.gd`(`Explosion.apply_aoe`:分段衰减/LOS遮挡/友伤),特效 `explosion.tscn` 纯视觉占位(播完自毁)。敌人子弹 `enemy_bullet.gd` 是带重力抛物线弹(`launch()`)。
